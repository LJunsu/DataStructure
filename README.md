...
+ ## 레드 블랙 트리 (Red Black Tree)
  ...
  + Recolor
    ...
    recolor 수행으로 상위 노드 간의 규칙이 어긋나면 재귀를 통해 구조를 유지하는 작업을 수행함

  + Restructure

    <pre>
      (이 트리는 잘못된 RB Tree지만, 설명을 위해 참고로만)
      ┌        B(할아버지) ┐
      ┌ R(부모) ┐           B(삼촌)
      R(나)     B
    </pre>
    ...
    나 < 부모 < 할아버지 라면 부모와 할아버지의 위치를 변경

    변경 과정에서 부모의 오른쪽은 할아버지의 왼쪽으로 보내고, 할아버지를 부모의 오른쪽으로 보내서 정렬

    할아버지.left = 부모.right

    부모.right = 할아버지
 
    이후 부모를 Black로, 나, 할아버지를 Red로
    ...
    5를 추가했다면 나.5 < 부모.6 < 항아버지.8 순서 (Left Left)
 
    할아버지의 left를 부모의 right인 B.7로
 
    부모의 right를 할아버지인 B.8로
    ...
    구조를 정렬한 후 부모를 Black로, 나, 할아버지를 Red로 변경
...
---
    ...
    7를 추가했다면 부모.6 < 나.7 < 항아버지.8 순서 (Left Right)
